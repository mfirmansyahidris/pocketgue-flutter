...
class CustomNestedScrollView extends StatefulWidget {
  ...
  /// custom functio to get callback if bottom almost reached
  final VoidCallback? onAlmostReachedBottom;
...
class CustomNestedScrollViewState extends State<CustomNestedScrollView> {
  ...
  void initState() {
    ...
    _coordinator?._innerController.addListener(() {
      if(((_coordinator?._innerController.position.maxScrollExtent ?? 0) - (_coordinator?._innerController.position.pixels ?? 0)) < 300){
        if(!_bottomCalled){
          widget.onAlmostReachedBottom?.call();
          _bottomCalled = true;
        }
      }else{
        _bottomCalled = false;
